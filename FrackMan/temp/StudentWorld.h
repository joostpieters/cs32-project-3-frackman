...
public:
	...
	GraphObject::Direction dirAndDistanceToFrackmanOrExit(int currX, int currY, bool destinationIsExit=false, int* distance = nullptr);
	...
	bool causeAnnoyanceWithinRadius(int x, int y, int changeInHitPoints);
...
private:
	...
	std::string formatDataField(int num, int width, char filler);
	...
	int m_probabilityHardCoreProtester;
};
